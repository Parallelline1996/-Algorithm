...
//        System.out.println(toGoatLatin("The quick brown fox jumped over the lazy dog"));
    ...
    private static String toGoatLatin(String S) {
        ...
        for (int i = 0; i < strings.length; i++) {
            if (isVowel(strings[i]) || (!isVowel(strings[i]) && strings[i].length() <= 1)) {
                sb.append(strings[i]);
                sb.append("m");
            } else {
    ...
    private static void addA (StringBuilder sb, int n) {
        for (int i = 0; i < n + 1; i++) {
            sb.append("a");
        }
    }
    ...
    private static boolean isVowel (String s) {
        ...
        return temp.equals("a") || temp.equals("e") || temp.equals("i")
